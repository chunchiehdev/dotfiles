...
get_confirmation() {
    ...
    if [ "$INTERACTIVE" = "false" ] && [ "$AUTO_MODE" = "true" ]; then
    ...
    while true; do
        ...
        if ! read -r result </dev/tty; then
            if [ "$AUTO_MODE" = "true" ]; then
                echo -e "${YELLOW}Cannot read from terminal. Using default value: $default_value${NC}"
                echo "$default_value"
                return 0
            else
                echo -e "${RED}ERROR: Cannot read from terminal. This script must be run interactively.${NC}"
                echo -e "${RED}Please download the script first and run it directly, or use AUTO_MODE=true:${NC}"
                echo -e "${BLUE}  curl -s https://raw.githubusercontent.com/chunchiehdev/dotfiles/main/init.sh -o init.sh${NC}"
                echo -e "${BLUE}  chmod +x init.sh${NC}"
                echo -e "${BLUE}  ./init.sh${NC}"
                echo -e "${BLUE}  # OR ${NC}"
